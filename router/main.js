...
module.exports = function(app) {
    ...
    var parseCommand = function(fcommand, fields, res, files) {
        ...
        if (cmdArray.length == 6) {
            if (cmd == mergein) {
                ...
                const LCA = findLCA(folderName, sourceSnapshot, targetSnapshot);
                console.log("LCA: " + LCA);
                mergeinFiles(fields, rep, sourceSnapshot, targetSnapshot, LCA, targetFolder, targetSnapshotName, files, res)
            }
        }
    ...
    //Function Merge in
    var mergeinFiles = function(fields, rep, sourceSnapshot, targetSnapshot, LCA, targetFolder, targetSnapshotName, files, res) {
        console.log("inside mergein now 1");
        const manifestpath = getmanifestpath(rep);
        ...
        const targetFolderPath = searchForMergeOut(contents, rep, sourceSnapshot, targetSnapshot, targetFolder);
        console.log(targetFolderPath)
        uploadMergeinFile(rep, fields, res, files, targetSnapshotName, targetFolder);
        console.log("inside mergein now 4");

    }

    // function to second+ upload file name
    var uploadMergeinFile = function(folderName, fields, res, files, userComment, sourceAdd) {
    ...
    var searchForMergeOut = function(contents, rep, sourceSnapshot, targetSnapshot, targetFolder) {
